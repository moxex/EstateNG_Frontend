    ...
    async function GetListingInfo() {
      try {
        const response = await Axios.get(
          `http://localhost:8000/api/v1/listings/${params.id}/`
        );
  ...
  useEffect(() => {
    if (state.listingInfo) {
      async function GetProfileInfo() {
        try {
          const response = await Axios.get(
            `http://localhost:8000/api/v1/profiles/${state.listingInfo.seller}/`
          );
    ...
    if (confirmDelete) {
      try {
        const response = await Axios.delete(
          `http://localhost:8000/api/v1/listings/${params.id}/delete/`
        );
    ...
    >
      ...
      >
        ...
        <Grid item container xs={5} alignItems="center">
          <Typography
            variant="h6"
            style={{ fontWeight: "bolder", color: "#ff6c38" }}
          >
      ...
      >
        ...
        {state.listingInfo.furnished ? (
          <Grid item xs={2} style={{ display: "flex" }}>
            <CheckBoxIcon style={{ color: "#ff6c38", fontSize: "2rem" }} />{" "}
